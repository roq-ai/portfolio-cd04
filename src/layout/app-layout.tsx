...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<PortfolioInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/portfolios` : null),
    () => getPortfolios({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Portfolio',
      path: isTenantUser ? `/portfolios/view/${data?.[0]?.id}` : '/portfolios',
      entity: 'portfolio',
    ...
    { name: 'Skill', path: '/skills', entity: 'skill', service: AccessServiceEnum.PROJECT, icon: FiStar },
    {
      name: 'Testimonial',
      path: '/testimonials',
      entity: 'testimonial',
